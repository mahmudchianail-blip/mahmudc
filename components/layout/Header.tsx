...
const Header: React.FC<HeaderProps> = ({ navigate, pathname }) => {
    ...
    return (
        <nav className={`fixed top-0 w-full z-50 px-6 py-4 transition-all duration-300 ${scrolled ? 'glass-effect' : 'bg-transparent'}`}>
            <div className="max-w-7xl mx-auto flex justify-between items-center relative">
                <a href="/" onClick={(e) => handleLinkClick(e, '/')} className="flex items-center gap-2 cursor-pointer">
                    <div className="w-10 h-10 bg-gradient-to-r from-purple-500 to-orange-500 rounded-full flex items-center justify-center font-bold text-lg">O</div>
                ...
                <div className="hidden md:flex gap-8 items-center absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
                    {navLinks.map((link) => (
                        <a 
                          ...
                          className={`relative font-medium transition-colors duration-300 ${activeId === link.id ? 'text-white' : 'text-gray-400'} hover:text-white`}
                        >
                            {link.title}
                            {activeId === link.id && <span className="absolute bottom-[-8px] left-0 w-full h-0.5 bg-purple-500"></span>}
                        </a>
                    ))}
                </div>
            ...
            <div className={`md:hidden fixed top-0 right-0 w-2/3 h-full glass-effect p-6 transition-transform duration-300 ease-in-out ${menuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
